  ...
  if (!outputPath) {
    if (process.platform === 'darwin') {
      ...
      outputPath = path.join(outputDir,
                             'TheHive\\ Browser\\ Development.app', 'Contents', 'MacOS',
                             'TheHive\\ Browser\\ Development')
    } else if (process.platform === 'win32') {
      outputPath = path.join(config.outputDir, 'thehive.exe')
    } else {
      outputPath = path.join(config.outputDir, 'thehive')
    }
  }
